...
// Package summonspets2_test reproduces the second set of "summons and pets" reports of the CoA server issue tracker.
// Each file covers one class and names the issue in its failure messages.
//
//	go test -tags=e2e ./e2e/coa/summonspets2 -count=1 -v -p 1
...
// distBetween returns the 3D distance between two visible units, or 999 when one is not visible.
func distBetween(bot *e2eharness.ScenarioBot, a, b uint64) float64 {
	oa, ob := bot.World.GetObject(a), bot.World.GetObject(b)
	if oa == nil || ob == nil {
		return 999
	}
	return float64(e2eharness.Distance3D(oa.PosX, oa.PosY, oa.PosZ, ob.PosX, ob.PosY, ob.PosZ))
}
